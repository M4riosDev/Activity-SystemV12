...
const marios = new Discord.Client()
const db = require('quick.db')
const config = require('./marios.json')

marios.on("ready", () =>{
    console.log(`${marios.user.tag} is online`)
})

marios.on('message', async (message) => {
	...
	const command = args.shift().toLowerCase();
        ...
        mariosoff = message.guild.channels.cache.get(config.kanali)
        ...
        if (command === 'on') {
            ...
            const marios = db.get(`timeof_${message.guild.id}_${message.author.id}`)
            if(!marios) return message.reply(`You are not on shift`)
            if(marios) {
                const end = new Date().getTime();
      
                const diff = end - marios;
      ...
      if(command == 'hours'){
      const marios = message.member.hasPermission(['ADMINISTRATOR'])
      const role = message.member.roles.cache.has(config['staff-team-role'])
      const user = message.mentions.members.first()
      
      
      if(user){
      if(!marios) return message.reply(`You are not Staff Manager to see hours of <@!${user.id}>`);
